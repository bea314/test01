...
export default function KitchenDisplayPage() {
  ...
  const getWaiterNameKDS = (waiterId: string) => {
    ...
    return waiter ? waiter.name.split(' ')[0] : waiterId; // Show first name or ID
  };
    ...
    setKdsOrders(prevOrders => 
      prevOrders.map(order => 
        order.id === orderId 
          ...
          : order
      ).map(order => ({ // Filter out items that are no longer pending/preparing
          ...order,
          items: order.items.filter(item => newStatus === 'cancelled' ? true : (item.status === 'pending' || item.status === 'preparing'))
      })).filter(order => order.items.length > 0) // Remove order if no items left for KDS
    );
              ...
              <CardHeader className="pb-3 pt-4 px-4">
                <div className="flex justify-between items-start">
                  <div>
                   ...
                   {/* Priority Indicator - example, could be based on time or order type */}
                  {new Date(order.createdAt).getTime() < currentTime.getTime() - 15 * 60 * 1000 && // Older than 15 mins
                ...
                {order.items.map(item => (
                  <div key={item.id} className="p-2 rounded-md border bg-background/70 hover:bg-muted/30">
                    ...
                    <div className="flex gap-1 mt-1.5 justify-end">
                        {item.status === 'pending' && (
                             <Button size="xs" variant="outline" onClick={() => handleItemStatusUpdate(order.id, item.id, 'preparing')}>Start Prep</Button>
                        )}
                        {item.status === 'preparing' && (
                             <Button size="xs" variant="outline" className="bg-green-500/20 hover:bg-green-500/30 text-green-500" onClick={() => handleItemStatusUpdate(order.id, item.id, 'ready')}>Mark Ready</Button>
                        )}
                    </div>
                  </div>
                ))}
